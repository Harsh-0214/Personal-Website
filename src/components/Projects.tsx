...
function ProjectCard({ project }: { project: typeof projects[0] }) {
  ...
  return (
    <div
      ...
      className="relative bg-card border border-line-subtle flex flex-col p-8 overflow-hidden group h-full"
    >
      {/* Dynamic glow spot */}
      {hovered && (
        <div
      ...
      <span className="font-mono text-[0.58rem] text-text-lo tracking-widest mb-6">{project.id} — {project.year}</span>

      <h3 className="font-display font-bold text-[1.3rem] leading-tight tracking-tight mb-3 group-hover:text-accent transition-colors">
        {project.title}
      </h3>

      <p className="font-mono text-[0.77rem] text-text-mid leading-relaxed flex-1 mb-6">
        {project.description}
      </p>

      <div className="flex flex-wrap gap-2 mb-6">
        {project.stack.map(s => (
          <span key={s} className="font-mono text-[0.6rem] text-accent px-2.5 py-1 bg-accent/10 border border-accent/20">
            {s}
          </span>
        ))}
      </div>
      ...
      {/* Highlight metric */}
      <div className="flex items-center gap-2 mb-6 py-3 border-t border-line-subtle">
  ...
  return (
    <section id="projects" className="bg-base">
      <div ref={ref} className="max-w-6xl mx-auto px-8 md:px-14 py-28">
        ...
        <div className="grid md:grid-cols-3 gap-px bg-line-subtle border border-line-subtle">
          {projects.map((p, i) => (
            <motion.div
              key={p.id}
